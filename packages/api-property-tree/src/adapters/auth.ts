...
assert(process.env.MSAL__CLIENT_ID, 'MSAL__CLIENT_ID is not set')
assert(process.env.MSAL__CLIENT_SECRET, 'MSAL__CLIENT_SECRET is not set')
assert(process.env.MSAL__TENANT_ID, 'MSAL__TENANT_ID is not set')
assert(process.env.MSAL__CLOUD_INSTANCE, 'MSAL__CLOUD_INSTANCE is not set')
assert(
  process.env.MSAL__POST_LOGOUT_REDIRECT_URI,
  'MSAL__POST_LOGOUT_REDIRECT_URI is not set',
)
assert(process.env.MSAL__REDIRECT_URI, 'MSAL__REDIRECT_URI is not set')

const redirectUri = process.env.MSAL__REDIRECT_URI
const postLogoutRedirectUri = process.env.MSAL__POST_LOGOUT_REDIRECT_URI
const cloudInstance = process.env.MSAL__CLOUD_INSTANCE
const tenantId = process.env.MSAL__TENANT_ID
const clientSecret = process.env.MSAL__CLIENT_SECRET
...
const msalConfig: msal.Configuration = {
  auth: {
    clientId: process.env.MSAL__CLIENT_ID, // 'Application (client) ID' of app registration in Azure portal - this value is a GUID
    authority: cloudInstance + tenantId, // Full directory URL, in the form of https://login.microsoftonline.com/<tenant>
    clientSecret: clientSecret, // Client secret generated from the app registration in Azure portal
...
const handleRedirect = (options: AuthOptions = defaultOptions) => {
  return async (ctx: Context) => {
    ...
    try {
      ...
      const state = JSON.parse(cryptoProvider.base64Decode(body.state))
